...
<?php foreach ($assets as $asset):?>
                <!--Teacher View Modal -->
                <div class="modal fade" id="<?php echo $asset->h_ID; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                  <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLongTitle"><?php echo $asset->h_name; ?></h5>
                      ...
                      <div class="modal-body">
                        ...
                        <li class="list-group-item active"><?php echo $asset->h_name; ?> &nbsp &nbsp &nbsp <small><?php echo $asset->h_stype; ?></small> </li>
                        <li class="list-group-item">Opening Balance:<?php echo $asset->h_opbalance; ?></li>
                        <li class="list-group-item">Current Balance:<?php echo $asset->h_balance; ?></li>
                      </ul>
                      </div>
...
@section('content')
   ...
   <h3 align="center" class="text-primary">
     View Record <small class="text-muted">(Assets)</small>
      </h3>
    <div class="card">
        <div class="card-header card-header-rose card-header-icon bg-primary">
          ...
          <h4 class="card-title text-white">Assets</h4>
          <?php 
           if(isset(Auth::user()->assetsadd) && Auth::user()->assetsadd == '1')
            { ?>
            <div align="right"><a href="{{action('AssetController@create')}}" class="btn btn-success btn-raised" >+ Add New</a>
          </div>
            <?php } else {} ?>
        </div>
        <div class="card-body">
            ...
            <div class="row">
              ...
              <tbody>
                ...
                <tr role="row" class="odd">
                  <td tabindex="0" class="sorting_1"><?php echo $asset->h_ID; ?></td>
                  <td><?php echo $asset->h_name; ?></td>
                  <td><?php echo $asset->h_stype; ?></td>
                  <td><?php echo $asset->h_opbalance; ?></td>
                  <td class="text-right" style="">
                   <?php 
                   if(isset(Auth::user()->assetsadd) && Auth::user()->assetsadd == '1')
                    { ?>
                    <a  class="btn btn-link btn-info btn-just-icon like" data-toggle="modal" data-target="#<?php echo $asset->h_ID; ?>" title="view"><i class="material-icons">visibility</i></a>
                     <?php } else {} ?>

                     <?php 
                   if(isset(Auth::user()->assetsedit) && Auth::user()->assetsedit == '1')
                    { ?>  
                    <a href="{{action('AssetController@edit', $asset->h_ID)}}" class="btn btn-link btn-warning btn-just-icon edit" title="edit"><i class="material-icons">edit</i></a>
                    <?php } else {} ?>

                     <?php 
                   if(isset(Auth::user()->assetsdelete) && Auth::user()->assetsdelete == '1')
                    { ?>  
                    <form action="{{action('AssetController@destroy', $asset->h_ID)}}" method="post" style="display: inline">
                      {{csrf_field()}}
                      <input name="_method" type="hidden" value="DELETE">
                      <input type="hidden" name="asname" value="<?php echo $asset->h_name; ?>">
