...
<!-- WORK AREA START -->
          ...
          <div class="col-md-4">Voucher#: <?php echo $details->bkvr_no; ?></div>
          <div class="col-md-4">ACC Title: <?php echo $details->acc_title; ?></div>
           <div class="col-md-4"> <?php  $newDate = date("d-M-Y", strtotime($details->created_at)); echo $newDate; ?> </div>
            <div class="col-md-4">Prepared By: <b><?php echo $details->bt_preparedby; ?></b></div>
           <?php endforeach; ?>
          <table id="datatables1" class="table table-striped table-bordered table-hover dataTable dtr-inline" cellspacing="0" width="100%" style="width: 100%;" role="grid" aria-describedby="datatables_info" border="1px">
              ...
              <tbody>
              <?php $total = 0; ?>
               <?php foreach ($voucher as $banktransactions):?>
                <tr role="row" class="odd">
                  <td><?php echo $banktransactions->bt_sno; ?></td>
                  <td><?php echo $banktransactions->ex_ID; ?></td>
                  <td><?php echo $banktransactions->bt_cqnumber; ?></td>
                  <td><?php echo $banktransactions->ex_name; ?></td>
                  <td><?php echo $banktransactions->bt_description; ?></td>
                  <td><?php $subtotal =  number_format($banktransactions->bt_amount, 2, '.', ',');
                    echo $subtotal; $total += $banktransactions->bt_amount; ?></td>
                </tr>
               <?php endforeach; ?>
                <tr>
                  <td></td>
       ...
       <!-- WORK AREA END -->
 <script>
            printDivCSS = new String ('<head> <title> Star Automation </title><link href="{{asset("css/bootstrap-material-design.min")}}" rel="stylesheet" type="text/css"> </head> <div> <img src="{{asset("images/star.png")}}" width="20%" height="auto"></div> <h3 align="center"> BANK PAYMENT </h3> ')
