...
<?php
  if(isset(Auth::user()->cashledgersview) && Auth::user()->cashledgersview == '1')
   ...
   <h3 align="center" class="text-primary">
     View Record <small class="text-muted">(Cash Ledger)</small>
      </h3>
    ...
    <div class="card">
        <div class="card-header card-header-rose card-header-icon bg-primary">
          ...
          <h4 class="card-title text-white">Cash Ledger</h4>
            ...
            <div class="row">
              ...
              <tbody>
                ...
                <tr role="row" class="odd">
                  <td tabindex="0" class="sorting_1">{{$voucher['cih_ID']}}</td>
                  <td>{{$voucher['cih_title']}}</td>
                  <td>{{$subtotal =  number_format($voucher['cih_obalance'], 2, '.', ',')}}/-</td>
                  <td>{{$subtotal =  number_format($voucher['cih_balance'], 2, '.', ',')}}/-</td>
                  <td>{{$voucher['created_at']}}</td>
                  <td>{{$voucher['updated_at']}}</td>
                  <td class="text-right" style="">
                        <a href="{{action('CashledgerController@show', $voucher['cih_title'])}}" class="btn btn-link btn-info btn-just-icon like" title="view">
                          <i class="material-icons">visibility</i>
                        </a>
                  </td>
                </tr>
                 <?php $total += $voucher['cih_balance']; ?>
                @endforeach
              </tbody>
            </table>
