...
<input type="hidden" name="counter" id="counter" value="">
          <?php foreach ($balances as $orbalance):?>
           <h3 align="center" class="text-primary">
            Cash Ledger (<?php echo $orbalance->cih_title; ?>)
          </h3>
          <br>

           <!--  <h4 class="text-rose">  </h4> -->
            <div class="form-inline" align="center">
            <div class="form-group" style="width: 50%">
            <label for="bk_branch_code" class="bmd-label-floating text-primary">Opening Balance</label>
            <input class="form-control" type='text' value="<?php $subtotal =  number_format($orbalance->ob_amount, 2, '.', ','); echo $subtotal; ?>" id='opbalance' name='opbalance' readonly style="width: 90%"/>
          ...
          <div class="form-group" style="width: 50%">
            ...
            <input class="form-control" type='text' value="<?php $subtotal =  number_format($orbalance->cih_balance, 2, '.', ','); echo $subtotal; ?>" id='crbalance' name='crbalance' readonly style="width: 90%"/>
          </div>
          </div>
         <!--  <form action="{{action('CashledgerController@rangeCal')}}" method="post" >
          {{csrf_field()}} -->
          <b> Enter a Range  </b>
          <div class="form-inline" align="center">
          <div class="form-group" style="width: 50%">
           ...
           <table style="width: 100%;" id="myTable" border=".5px" class="table table-striped table-bordered table-hover dataTable dtr-inline">
              <thead class="bg-primary">
                <tr class="bg-primary header text-white" style="text-align: left">
                  ...
                  <th style="width: 10px;font-size: 10px">Date</th>
                  <th style="width: 10px;font-size: 10px">Voucher #</th>
                  <th style="width: 10px;font-size: 10px">Name</th>
                  <th style="width: 10px;font-size: 10px">Description</th>
                  <th style="width: 20px;font-size: 10px">Debit</th>
                  <th style="width: 20px;font-size: 10px">Credit</th>
                  <th style="width: 20px;font-size: 10px">Balance</th>
                  <th style="width: 20px;font-size: 10px">Action</th>
                </tr>
              </thead>
              <tbody>
               ...
               <?php foreach ($voucher as $cashtransactions):?>
                <tr style="font-size: 10px" class="line[]">
                  ...
                  <td><input type="hidden" name="debit[]" value="-"> <span>-</span></td>
                   ...
                   <td>
                  <input type="hidden" name="credit[]" value="<?php echo $cashtransactions->ct_amount;  ?>">
                  <span><?php $subtotal =  number_format($cashtransactions->ct_amount, 2, '.', ',');
                    echo $subtotal; $totalcredit += $cashtransactions->ct_amount; ?></span>
                   </td>

                  <td> <span class="balance"></span> <input type="hidden" name="balance[]"></td>

                  <td class="hid">
                     <?php if($cashtransactions->ct_name === ""){ ?>
                     <a  href="{{action('JvController@show', $cashtransactions->vr_no)}}" style="font-size: 10px">view</a>
                    <?php } else { ?>
                     <a  href="{{action('CashtransactionController@show', $cashtransactions->vr_no)}}" style="font-size: 10px">view</a>
                    <?php } ?>
                  </td>
                </tr>
                <?php endforeach; ?>
               ...
               <?php foreach ($rvoucher as $rcashtransactions):?>
                 <tr style="font-size: 10px" class="line[]">
                  ...
                  <td>
                  <input type="hidden" name="debit[]" value="<?php echo $rcashtransactions->cr_amount;  ?>">
                  <span><?php $subtotal =  number_format($rcashtransactions->cr_amount, 2, '.', ',');
                    echo $subtotal; $totaldebit += $rcashtransactions->cr_amount; ?></span>
                   </td>
                 ...
                 <td><input type="hidden" name="credit[]" value="-"> <span>-</span></td>
                  ...
                  <td class="hid">
                     <?php if($rcashtransactions->cr_name === ""){ ?>
                     <a  href="{{action('JvController@show', $rcashtransactions->crv_no)}}" style="font-size: 10px">view</a>
                    <?php } else { ?>
                     <a  href="{{action('CashreceiptController@show', $rcashtransactions->crv_no)}}" style="font-size: 10px">view</a>
                    <?php } ?>
                  </td>
               </tr>
               <?php endforeach; ?>
...
// Top Current End Here //
            printDivCSS = new String ('<head> <title> Star Automation </title><link href="{{asset("css/bootstrap-material-design.min")}}" rel="stylesheet" type="text/css"> </head> <div> <img src="{{asset("images/star.png")}}" width="20%" height="auto"></div> <br> <span style="font-size:11px"><?php echo $orbalance->cih_title; ?> <br> Opening Balance: <?php $subtotal =  number_format($orbalance->ob_amount, 2, '.', ','); echo $subtotal; ?> <br>Current Balance: <?php $subtotal =  number_format($orbalance->cih_balance, 2, '.', ','); echo $subtotal; ?></span> <br>')
