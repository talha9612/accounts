 ...
 <input type="hidden" name="counter" id="counter" value="">
  <?php foreach ($assetbalances as $assetorbalance):?>
           <h3 align="center" class="text-primary">
            Expense Ledger (<?php echo $assetorbalance->h_name; ?> )
          </h3>
          <br>
         ...
         <div id="div1">
           <table style="width: 100%;" id="myTable" border=".5px" class="table table-striped table-bordered table-hover dataTable dtr-inline">
              ...
              <tbody>
               ...
               <?php if($assetorbalance->h_name == 'Import GST & A. GST'){ ?>
                 <?php foreach ($gst as $saletax):?>
                   <?php if($saletax->sq_totalst == '0'){} else{ ?>
                 <tr style="font-size: 10px" class="line[]">
                  <td><?php echo $saletax->created_at; ?></td>
                  <td><?php echo $saletax->sq_number; ?></td>
                  <td></td>
                  <td><?php echo $saletax->fr_name; ?></td>
                  <td><?php echo $saletax->sq_title; ?></td>
                  <td><input type="hidden" name="debit[]" value="-">
                    <span>-</span>
                  </td>
                  <td>
                  <input type="hidden" name="credit[]" value="<?php echo $saletax->sq_totalst;  ?>">
                  <span><?php $subtotal =  number_format($saletax->sq_totalst, 2, '.', ',');
                    echo $subtotal; $totalcredit += $saletax->sq_totalst; ?></span>
                   </td>
                  <td> <span class="balance"></span> <input type="hidden" name="balance[]"></td>
