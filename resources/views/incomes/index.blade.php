...
@section('content')
   ...
   <h3 align="center" class="text-primary">
     View Record <small class="text-muted">(Incomes)</small>
      </h3>
    <div class="card">
        <div class="card-header card-header-rose card-header-icon bg-primary">
          ...
          <h4 class="card-title text-white">Incomes</h4>
           <?php
          if(isset(Auth::user()->incomeadd) && Auth::user()->incomeadd == '1')
              { ?>
          <div align="right"><a href="{{action('IncomeController@create')}}" class="btn btn-success btn-raised" >+ Add New</a>
          </div>
            <?php } else {} ?>
        </div>
        <div class="card-body">
                  ...
                  <td><?php echo $expense->h_stype; ?></td>
                   ...
                   <?php
                    ...
                    if(isset(Auth::user()->incomeedit) && Auth::user()->incomeedit == '1')
                        { ?>
                    <a href="{{action('IncomeController@edit', $expense->h_ID)}}" class="btn btn-link btn-warning btn-just-icon edit" title="edit"><i class="material-icons">edit</i></a>
                     <?php } else {} ?>

                      <?php
                    if(isset(Auth::user()->incomedelete) && Auth::user()->incomedelete == '1')
                        { ?>
                    <form action="{{action('IncomeController@destroy', $expense->h_ID)}}" method="post" style="display: inline">
                      {{csrf_field()}}
                      <input name="_method" type="hidden" value="DELETE">
                      <input type="hidden" name="asname" value="<?php echo $expense->h_name; ?>">
