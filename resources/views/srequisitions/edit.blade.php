...
<!-- WORK AREA START -->
           <h3 align="center" class="text-primary">
            Edit Record <small class="text-muted">(Sale Requisition)</small>
          </h3>
          <br>
          ...
          <div class="container">
            <div class="form-inline">
                <div class="form-group">
                          ...
                          <input class="form-control" type='text' name='sr_number' value="<?php echo $details->sr_number; ?>" readonly required/>
                          <input class="form-control" type='hidden' name='sr_req' value="1" readonly/>
                </div>
                <div class="form-group" style="margin-left: 65%">
                          <input class="form-control" type='text' name='updated_at' value="<?php echo $details->updated_at; ?>" readonly required/>
                </div>
              </div> 
                   
                    <div class="form-group">
                          <label for="bk_name" class="bmd-label-floating text-primary">Name</label>
                          <input class="form-control" type='text' id='pr_name' name='sr_name' value="<?php echo $details->sr_name; ?>" required/>
                    </div>
                    <div class="form-group">
                          <label for="bk_branch_code" class="bmd-label-floating text-primary">Req Title</label>
                          <input class="form-control" type='text' id='sr_title' name='sr_title' value="<?php echo $details->sr_title; ?>" required/> 
                    </div>
              </div>
              {{csrf_field()}}
              <?php endforeach; ?>
           <table id="datatables" class="table table-striped table-bordered table-hover dataTable dtr-inline" cellspacing="0" width="100%" style="width: 100%;" role="grid" aria-describedby="datatables_info">
              ...
              <tbody>
               <?php foreach ($requisition as $requisition):?>
                <tr role="row" class="odd">
                  <td><input type="text" name="sr_supplier[]" class="form-control" value="<?php echo $requisition->sr_supplier; ?>" required> </td>
                  <td><input type="text" name="sr_item[]" class="form-control" value="<?php echo $requisition->sr_item; ?>" required> </td>
                  <td><input type="text" name="sr_size[]" class="form-control" value="<?php echo $requisition->sr_size; ?>" required></td>
                  <td><input type="text" name="sr_specifications[]" class="form-control" value="<?php echo $requisition->sr_specs; ?>" required> </td>
                  <td><input type="text" name="sr_description[]" class="form-control" value="<?php echo $requisition->sr_description; ?>" required></td>
                  <td><input type="text" name="sr_quantity[]" class="form-control" value="<?php echo $requisition->sr_quantity; ?>" required>
                  </td>
                </tr>
               <?php endforeach; ?>
              </tbody>
             ...
             <div align="center" >
               <a href="{{action('SrequisitionController@savedSr')}}" class="btn btn-warning btn-raised">Cancel</a>
