...
<!-- WORK AREA START -->
           <h3 align="center" class="text-primary">
            View Record <small class="text-muted">(Sale Requisition)</small>
          </h3>
          <br>
          ...
          <div class="container">
            <div class="form-inline">
                <div class="form-group">
                          ...
                          <input class="form-control" type='text' name='pr_number' value="<?php echo $details->sr_number; ?>"  readonly/>
                </div>
                <div class="form-group" style="margin-left: 65%">
                    ...
                    <div class="form-group">
                          ...
                          <input class="form-control" type='text' id='pr_name' name='pr_name' value="<?php echo $details->sr_name; ?>" readonly/>
                    </div>
                    <div class="form-group">
                          <label for="bk_branch_code" class="bmd-label-floating text-primary">Req Title</label>
                          <input class="form-control" type='text' id='pr_title' name='pr_title' value="<?php echo $details->sr_title; ?>" readonly/> 
                    </div>
                     <div class="form-group" align="right">
                    <a href="{{action('SrequisitionController@printSr' , $details->sr_number)}}" target="new" title="Print/PDF"> <i class="material-icons text-warning">local_printshop</i> </a> 
                    </div>
              </div>
              <?php endforeach; ?>
           <table class="table table-striped table-bordered table-hover dataTable dtr-inline" cellspacing="0" width="100%" style="width: 100%;" role="grid" aria-describedby="datatables_info">
              ...
              <tbody>
               <?php foreach ($requisition as $requisition):?>
                <tr role="row" class="odd">
                  <td><?php echo $requisition->sr_supplier; ?></td>
                  <td><?php echo $requisition->sr_item; ?></td>
                  <td><?php echo $requisition->sr_size; ?></td>
                  <td><?php echo $requisition->sr_specs; ?></td>
                  <td><?php echo $requisition->sr_description; ?></td>
                  <td>
                    <input type="text" name="sr_quantity[]" class="form-control" value="<?php echo $requisition->sr_quantity; ?>" readonly>
                    <span style="display: none"><?php echo $requisition->sr_quantity; ?></span>
                  </td>
                </tr>
               <?php endforeach; ?>
              </tbody>
             ...
             <?php 
              if(isset(Auth::user()->role) && Auth::user()->role == 'Admin')
                                     { ?>
            <form action="{{action('SquotationController@create', $details->sr_number)}}" method="get" align="center" 
              <?php if($requisition->sr_status=="2" || $requisition->sr_status=="0")  {echo "hidden";} else{} ?>>
