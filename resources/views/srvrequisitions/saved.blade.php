...
<?php
  if(isset(Auth::user()->salesview) && Auth::user()->salesview == '1')
   ...
   <h3 align="center" class="text-primary">
     Saved <small class="text-muted">(Sale Requisitions)</small>
      </h3>
    <div class="card">
        <div class="card-header card-header-rose card-header-icon bg-primary">
          ...
          <h4 class="card-title text-white">Sale Requisitions</h4>
           <div align="right">
             <a href="{{action('SrequisitionController@index')}}" class="btn btn-success btn-raised" >Posted SR's</a>
               <?php 
              ...
              <tbody>
                ...
                <tr role="row" class="odd">
                  ...
                  <td class="text-right" style="">
                      ...
                      if(isset(Auth::user()->role) && Auth::user()->role == 'Admin')
                                     { ?>
                        <a href="{{action('SrequisitionController@edit', $requisition->sr_number)}}" class="btn btn-link btn-warning btn-just-icon edit" title="edit/view"><i class="material-icons">visibility</i></a>

                         <?php } else {} ?>

                          <form action="{{action('SrequisitionController@saveSr', $requisition->sr_number)}}" method="get" style="display: inline;">
                            {{csrf_field()}}
                           ...
                           <?php 
                            if(isset(Auth::user()->role) && Auth::user()->role == 'Admin')
                                     { ?>
                          <form action="{{action('SrequisitionController@destroy', $requisition->sr_number)}}" method="post" style="display: inline">
                          {{csrf_field()}}
